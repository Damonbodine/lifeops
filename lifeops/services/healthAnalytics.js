...
class HealthAnalytics {
  ...
  async generateHealthBrief() {
    ...
    // Create more detailed context for AI
    const dataContext = this.createDetailedDataContext(recent7Days, daily7);
    const summary7 = this.generateHealthSummary(daily7);
    const summary30 = this.generateHealthSummary(daily30);

    const briefPrompt = `
    You have access to comprehensive health data from the last 7 and 30 days. Create a personalized daily health brief.

    RECENT 7-DAY HEALTH SUMMARY:
    - Steps: ${summary7.steps ? Math.round(summary7.steps.dailyAverage) + ' avg daily, trend: ' + summary7.steps.trend : 'No data'}
    - Heart Rate: ${summary7.heartRate ? Math.round(summary7.heartRate.dailyAverage) + ' bpm avg, trend: ' + summary7.heartRate.trend : 'No data'}
    - Active Energy: ${summary7.activeEnergy ? Math.round(summary7.activeEnergy.dailyAverage) + ' cal avg, trend: ' + summary7.activeEnergy.trend : 'No data'}
    - HRV: ${summary7.hrv ? Math.round(summary7.hrv.dailyAverage) + ' ms avg, trend: ' + summary7.hrv.trend : 'No data'}
    - Data points: ${summary7.steps ? summary7.steps.dataPoints + ' days' : 'Limited'}

    30-DAY COMPARISON:
    - Steps: ${summary30.steps ? Math.round(summary30.steps.dailyAverage) + ' avg' : 'No data'}
    - Heart Rate: ${summary30.heartRate ? Math.round(summary30.heartRate.dailyAverage) + ' bpm avg' : 'No data'}
    - Active Energy: ${summary30.activeEnergy ? Math.round(summary30.activeEnergy.dailyAverage) + ' cal avg' : 'No data'}

    DETAILED CONTEXT:
    ${dataContext}

    Based on this REAL health data, provide a brief morning briefing (2-3 sentences) that includes:
    1. A specific observation about recent performance vs 30-day average
    2. One notable trend (be specific with numbers)
    3. One actionable recommendation for today

    Be encouraging, specific, and data-driven. Use the actual numbers provided.
    `;
    ...
    const completion = await this.openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert health coach analyzing real health data. Provide specific, data-driven insights using the actual numbers provided. Be encouraging and actionable."
        },
        {
          role: "user",
          content: briefPrompt
